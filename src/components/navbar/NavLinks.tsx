...
export const NavLinks = ({ label, subItem }: NavLinkProps) => {
  ...
  return (
    <li
      className="text-lightBlack text-[13px] font-poppins py-3 hover:cursor-pointer transition-all trasition-duration-200 relative group">
      <span>{label}</span>
      ...
      <div className="absolute left-0 mt-6 w-52 p-2 bg-white border rounded-lg shadow-lg opacity-0 group-hover:opacity-100 pointer-events-auto group-hover:pointer-events-auto transition-opacity duration-300 z-10">
        <div className="">
          {subItem.map((item) => (
            <p className="w-full flex items-center text-md text-gray-600 p-2 hover:bg-lightSky rounded relative">
              {item}
